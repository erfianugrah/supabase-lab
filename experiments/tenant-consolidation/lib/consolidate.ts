/**
 * Shared mechanics for the consolidation experiment.
 *
 ...
 *
 * The cross-project copy here is the same shape the shared-tenancy runbook
 * prescribes - enumerate non-generated columns from the catalog, ship the rows
 * as JSON, land them with `json_populate_recordset` - because the point is to
 * test THAT procedure in the many-to-one direction, not a tidier one nobody
 * runs.
 */
 ...
 * INSERT ... RETURNING whose new row the SELECT policy hides fails with the
 * same 42501 as an INSERT the WITH CHECK expression refused. Any test that
 * wants to know whether the WRITE was allowed has to ask for `minimal` and
 * then count rows server-side.
 */
